...
class Firma
{
            ...
            if (openssl_pkcs12_read(file_get_contents($pathFirma, FILE_USE_INCLUDE_PATH), $certs, $this->config['pass'])) {
                ...
                if ($certificante === "BANCO CENTRAL DEL ECUADOR") {
                    ...
                    foreach ($aux as $item) {
                        ...
                        if (trim($subject) === trim($certData['subject']['CN'])) {
                    ...
                    //$aux = $certs['cert'];
                    //$x509cert = openssl_x509_read($aux);
                    //$certData = openssl_x509_parse($x509cert);
    ...
    public function firmar($xml, $docFirmados)
    {
        ...
        try {
            ...
            if (strpos($xml, 'standalone="yes"') !== false)
                $xml = str_replace('<?xml version="1.0" encoding="UTF-8" standalone="yes"?>' . PHP_EOL, "", $xml);
            else
                $xml = str_replace('<?xml version="1.0" encoding="UTF-8"?>' . PHP_EOL, "", $xml);
    ...
    {
        //$pkey = openssl_x509_read($this->cert);
    ...
    private function setSignTime($time)
    {
